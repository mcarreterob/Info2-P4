...
package body Maps_G is
   ...
   begin
	...
	function Max (M:Map) return Map is
	begin
   ...
   begin
	...
	function Map_Depth_Max (M: Map) return Natural is
		N: Natural := 1;
		P: Natural := 1;

	begin
		if M.Left /= null then
			N := Map_Depth_Max(M.Left) + 1;
		end if;
		if M.Right /= null then
			P := Map_Depth_Max(M.Right) + 1;
		end if;
		if P > N then
			return P;
		elsif N > P then
	...
	function Map_Depth_Min (M: Map) return Natural is
		N: Natural := 1;
		P: Natural := 1;
	begin
		if M.Left /= null then
			N := Map_Depth_Min(M.Left) + 1;
		end if;
		if M.Right /= null then
			P := Map_Depth_Min(M.Right) + 1;
		end if;
		if P > N then
			return N;
		elsif N > P then
   ...
   begin
	...
	procedure Recorrido(M: in out Map; C: out Cursor) is
		X: Map;
	begin
		
		C.M := Min(M);
		if C.M.Right /= null then
			C.M := C.M.Right;
		end if;
		C.Salto := C.Salto + 1;
		X := Delete_Min(M);
